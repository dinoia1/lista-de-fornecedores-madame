...
A API valida os campos, exige autorização e Origin exata do site, limita o corpo a 8 KB e aplica limite de 10 tentativas por 10 minutos por visitante. O limite usa memória e reinicia com o processo. Atrás de proxy, configure o IP exato em `TRUSTED_PROXY_IPS` e sobrescreva `X-Forwarded-For` para distinguir visitantes com segurança. Nenhum endereço IP é salvo nos cadastros. O campo invisível `website` rejeita preenchimento automatizado simples. A validação não verifica propriedade do e-mail ou telefone. A implantação VPS inclui backup local criptografado; veja `DEPLOY-VPS.md`.
